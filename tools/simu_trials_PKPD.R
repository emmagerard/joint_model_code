...
create_model_Chen = function(names_PD_param,fixed_PD_param,n_admin=7){
  PKPD_model = paste0(
...
######Simu one trial
data_simu=function(id,doses,t,t_doses,t_inf,
  ...
  for(i in 1:nrow(doses)){
    ...
    tox=((alpha*Rmax_true)>=threshold)*1
...
algo_trial=function(itrial,DL,t,t_doses,t_inf,
                    PK_model,PK_param_pop,
                    PD_model,PD_param_pop,
                    b_PK,b_PD,PD_initial_values,method_ode,
                    threshold,w_alpha){
  ...
  while(!MTDTrouvee){  
    ...
    if(is.null(data_PKPD)){
      ...
      id=data_PKPD$ID[length(data_PKPD$ID)]+1:3
    }
    
    simu_while=data_simu(id=id,doses=DL[rep(current_dose,3),],t=t,t_doses=t_doses,
                         t_inf=t_inf[rep(current_dose,3),],
    ...
    data_tox$dose=rep(current_dose,nrow(data_tox))
    ...
    if(sum(tox_while_tot)==1){
      ...
      simu_while2=data_simu(id=id,doses=DL[rep(current_dose,3),],t=t,t_doses=t_doses,
                            t_inf=t_inf[rep(current_dose,3),],
                            PK_model=PK_model,PK_param_pop=PK_param_pop,
                            PD_model=PD_model,PD_param_pop=PD_param_pop,
                            b_PK=b_PK,b_PD=b_PD,PD_initial_values=PD_initial_values,method_ode=method_ode,
      ...
      data_while2=simu_while2$data_PKPD
      
      data_tox2=simu_while2$data_tox
      data_tox2$dose=rep(current_dose,nrow(data_tox2))
      
      tox_while_tot2=tapply(data_tox2$tox,data_tox2$id,max)
      
      data_PKPD=rbind(data_PKPD,data_while2)
      data=rbind(data,data_tox2)
      
      if((sum(tox_while_tot)+sum(tox_while_tot2))<=1){
  ...
  #######
  return(list(data_PKPD=data_PKPD,data=data,MTD=current_dose))
}
...
crm_2p_trial=function(itrial,DL,t,t_doses,t_inf,
  ...
  while(n_patients+coh_size <= n_tot && current_dose > 0){
    ...
    simu_while=data_simu(id=id,doses=DL[rep(current_dose,coh_size),],t=t,t_doses=t_doses,
...
simu_one_trial_PKPD=function(itrial,DL,t,t_doses,t_inf,
  ...
  if(design=="3p3"){
    trial_data=algo_trial(itrial=itrial,DL=DL,t=t,t_doses=t_doses,t_inf=t_inf,
                          PK_model=PK_model,PK_param_pop=PK_param_pop,
                          PD_model=PD_model,PD_param_pop=PD_param_pop,
                          b_PK=b_PK,b_PD=b_PD,PD_initial_values=PD_initial_values,method_ode=method_ode,
                          threshold=threshold,w_alpha=w_alpha)
    ...
    write.table(MTD,
                file = "MTD.txt",row.names = FALSE, col.names = FALSE)
...
simu_trials_PKPD=function(n_trials,DL,t,t_doses,t_inf,
  ...
  for(i in 1:n_trials){
    ...
    simu_one_trial_PKPD(itrial=i,DL=DL,t=t,t_doses=t_doses,t_inf=t_inf,
                        PK_model=PK_model,PK_param_pop=PK_param_pop,
                        PD_model=PD_model,PD_param_pop=PD_param_pop,
...
simu_trials_PKPD_parallel=function(n_trials,DL,t,t_doses,t_inf,
  ...
  foreach(ip=1:N_pack,.packages=c("deSolve","rstan","MASS","dfcrm","lixoftConnectors"),.export=ls(globalenv())) %dopar%{
    ...
    for(i in 1:N_sim){
      ...
      simu_one_trial_PKPD(itrial=(ip-1)*N_sim+i,DL=DL,t=t,t_doses=t_doses,t_inf=t_inf,
                          PK_model=PK_model,PK_param_pop=PK_param_pop,
                          PD_model=PD_model,PD_param_pop=PD_param_pop,
                          fixed_param=fixed_param,param_var=param_var,init=init,
                          b_PK=b_PK,b_PD=b_PD,PD_initial_values=PD_initial_values,method_ode=method_ode,
